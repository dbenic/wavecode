...
### Added
- Per-run orchestrate result file at `{workspace}/.wavecode/runs/<runId>/result.txt` (or `{transcripts_root}/runs/<runId>/result.txt` without a workspace). Last line is exactly `RESULT: PASS` or `RESULT: FAIL` plus a one-line reason. Exposed on `GET /api/tasks/:id` runs, `GET /api/tasks/:id/runs`, and `GET /api/runs/:id/result`. Spawned/adopted prompts are briefed to write the file before idle. Missing, unparseable, or pane-only text is never PASS — idle-close writes `RESULT: FAIL`. Referee / wavepulse-gate RESULT remains the promote gate.

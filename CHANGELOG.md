...
### Added
- Per-run orchestrate result file (append-only local buffer) at `{workspace}/.wavecode/runs/<runId>/result.txt` (or `{transcripts_root}/runs/<runId>/result.txt` without a workspace). Last line is exactly `RESULT: PASS` or `RESULT: FAIL` plus a one-line reason. The file is the source of truth; `GET /api/tasks/:id` runs, `GET /api/tasks/:id/runs`, and `GET /api/runs/:id/result` expose path/contents as a convenience. Spawned/adopted prompts are briefed to append the file before idle. Missing, unparseable, or pane-only text is never PASS — WaveCode may append `RESULT: FAIL` or leave the file missing. Referee / wavepulse-gate RESULT remains the promote gate.

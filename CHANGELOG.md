...
### Fixed
- Spawned agents that go stably idle (finished pane / idle prompt) now close their running run via the same `onRunComplete` path as adopted agents. Grok `Responding` / `Thinking` counts as working so a generating seat is not marked idle. The watcher closes the pane-named `run_id` (echo|nc + RESULT) or an older stuck run when the agent has moved on — not only `currentRunId` — without a daemon restart. `wavecode tasks` no longer throws EPIPE when a consumer closes the pipe.
